...
const quickActions = ["Quiz me", "Hint", "Review"];
...
function demoReply(value) {
  const chosen = quickActions.find((item) => value.toLowerCase().includes(item.toLowerCase())) || 'Practice Coach';
  return `${chosen} mode: I can help with local model testing and prompt-result notes; practical idea sprint support; clear next-step guidance. Ask one specific question and I will turn it into a useful action. Requested focus: "${value}".`;
}
...
document.querySelectorAll('.chip').forEach((button) => {
  button.addEventListener('click', () => {
    ...
    text.focus();
    qualityEl.textContent = `Premium variant: insight-deck / ${button.textContent}`;
  });
});
...
add("I can help with local model testing and prompt-result notes; practical idea sprint support; clear next-step guidance. Ask one specific question and I will turn it into a useful action.");
